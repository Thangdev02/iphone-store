...
import './profilePage.css'; // External CSS file for custom styles (optional)

const ProfilePage = () => {
  const user = JSON.parse(localStorage.getItem('user'))[0]; // Retrieve the user from localStorage
  const [editableUser, setEditableUser] = useState(user); // Create a state for editable user
  const navigate = useNavigate();

  if (!user) {
    return <h2 className="text-center mt-5">No user information available</h2>;
  }
  ...
  const handleSave = () => {
    // Save the updated data to localStorage (could also be sent to a server)
